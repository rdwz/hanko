...
class WebauthnClient extends AbstractClient {
  ...
  /**
   * Performs a WebAuthn authentication ceremony and stores the used credential to the local storage. When 'userID' is
   * specified, the API is able to provide a list of allowed credentials and the browser presents a list of suitable
  ...
  /**
   * Performs a WebAuthN registration ceremony and stores the used credential to the local storage.
   *
   * @return {Promise<void>}
   * @throws {WebAuthnRequestCancelledError}
   * @throws {RequestTimeoutError}
   ...
   * authenticator is available and the user's credentials do not intersect with the credentials already stored in local
   * storage.
   *
...
class PasscodeClient extends AbstractClient {
  ...
  /**
   * Validates the passcode obtained from the email against the API.
   *
   * @param {string} userID - The UUID of the user.
